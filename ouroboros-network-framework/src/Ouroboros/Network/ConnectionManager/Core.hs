...
data DemoteToColdLocal peerAddr handlerTrace handle handleError version m
    ...
    | PruneConnections        (ConnectionId peerAddr)
                              ...
                              -- Left case is for when pruning tries to prune the connection which
                              -- triggered pruning, in this case we do not want to trace a
                              -- new transition.
                              --
                              -- Right case is for when the connection which triggered
                              -- pruning isn't pruned. In this case we do want to trace a
                              -- new transition.
...
withConnectionManager ConnectionManagerArguments {
  ...
  where
    ...
    includeInboundConnectionImpl
        ...
        -- includeInboundConnection can run safely without going above the hardlimit.
        -- So we want to make a check after locking the TMVar with the
        -- ConnectionManagerState and only then decide if given the current value of the
        -- number of incomingConns enables us to really include a new inbound
        -- connection.
        -> socket
    ...
    unregisterInboundConnectionImpl stateVar peerAddr = mask $ \restore -> do
          ...
          Just connVar -> do
            ...
            case connState of
              ...
              OutboundDupState connId connThread handle Ticking -> do
                ...
                -- actually let this connection to evolve if we need to make
    ...
    promotedToWarmRemoteImpl
        ...
        -> peerAddr
        -> m (OperationResult AbstractState)
    promotedToWarmRemoteImpl stateVar peerAddr = mask $ \restore -> do
          ...
          Just connVar -> do
            ...
            case connState of
              ...
              OutboundDupState connId connThread handle _expired -> do
                ...
                -- @
                --
                -- For connections that reach DuplexState we are not sure if this was
                -- due a connection that was established due to TCP simultaneous open or
                -- normal connect/accept. If it was established due to TCP simultaneous
                -- open a DuplexState can make us go above the Server number of connections
                -- hard limit, hence we need to prune connections.
                --
                let connState' = DuplexState connId connThread handle
                    tr = mkTransition connState connState'
                ...
                then do
                  ...
                  -- Since this connection might be a Sim-open connection and
                  -- we are above the hardlimit, we really need to prune this
                  -- connection in order to preserve the hard limit, so this
                  -- connection needs to be included in its own pruneSet.
                  let pruneSet' = Set.insert peerAddr pruneSet
      ...
      case result of
        ...
        _ -> pure ()
      restore $ return (abstractState . fromState <$> fst result)
