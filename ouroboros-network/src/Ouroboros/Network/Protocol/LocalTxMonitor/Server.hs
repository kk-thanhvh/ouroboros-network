...
-- | In the 'StIdle' protocol state, the server does not have agency. Instead, it
-- is waiting for:
--
-- * An acquire request
...
-- | In the 'StAcquiring' protocol state, the server has agency and must acquire, and hold
-- on to, the current / latest snapshot of its mempool.
--
...
-- | In the 'StAcquired' protocol state, the server does not have agency and is waiting for
-- a client to either:
--
...
-- request. The state is parameterized by a kind 'StBusyKind' to highlight the fact that, the
-- server is in a busy state in response to a particular query, and only responses for this query
